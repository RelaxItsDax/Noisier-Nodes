...
void Voronoi2D_Periodic_float(float2 UV, float AngleOffset, float CellDensity, out float Out, out float Cells) {
    float2 g = floor(UV * CellDensity);
    float2 f = frac(UV * CellDensity);
    float3 res = float3(8.0, 8.0, 8.0);
    ...
    for(int y=-1; y<=1; y++){
        for(int x=-1; x<=1; x++){
            float2 lattice = float2(x, y);
            float2 neighborPos = g + lattice;
            float2 wrappedPos = fmod(neighborPos, CellDensity);
            wrappedPos = (wrappedPos < 0.0) ? wrappedPos + CellDensity : wrappedPos;
            float2 offset = voronoi_noise_randomVector(wrappedPos, AngleOffset);
            float2 v = lattice + offset - f;
            float d = dot(v, v);
