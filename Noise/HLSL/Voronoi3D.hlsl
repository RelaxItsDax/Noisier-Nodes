...
void Voronoi3D_Periodic_float(float3 UV, float AngleOffset, float CellDensity, out float Out, out float Cells) {
    float3 g = floor(UV * CellDensity);
    float3 f = frac(UV * CellDensity);
    float3 res = float3(8.0, 8.0, 8.0);
    ...
    for(int y=-1; y<=1; y++){
        for(int x=-1; x<=1; x++){
            for(int z=-1; z<=1; z++){
                float3 lattice = float3(x, y, z);
                float3 neighborPos = g + lattice;
                float3 wrappedPos = fmod(neighborPos, CellDensity);
                wrappedPos = (wrappedPos < 0.0) ? wrappedPos + CellDensity : wrappedPos;
                float3 offset = voronoi_noise_randomVector(wrappedPos, AngleOffset);
                float3 v = lattice + offset - f;
                float d = dot(v, v);
